...
def generate_chat_title(user_message: str) -> str:
    ...
    title_prompt = f"Generate a concise, 3-to-4 word title summarizing this request. Reply ONLY with the title, no quotes or extra text. Request: {user_message}"
    try:
        # Explicitly convert the LLM output to a string so Pylance stops complaining
        raw_content = llm.invoke(title_prompt).content
        title = str(raw_content).strip()
        
        # Clean up any stray quotes the LLM might include
